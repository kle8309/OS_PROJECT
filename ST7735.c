...
void ST7735_OutChar(uint8_t ch){
  ...
  StX++;
	// limits the char along the x.  originally StX limit was 20
	// changed to landscape so now it's 26
  if(StX>26){
    StX = 26;
    ST7735_DrawCharS(StX*6,StY*10,'*',ST7735_RED,ST7735_BLACK, 1);
  }
...
//------------ST7735_Message------------
// Extend the device driver for the LCD so that there are two logically separate displays, one display using the top
// half and one display for the bottom half. There should be at least 4 lines per display.
// Input: device   specifies top or bottom
//        line     specifies the line number (0 to 3),
//        string   is a pointer to a null-terminated ASCII string
//        value    is a number to display.
// Output: none
...
void ST7735_Message(int device, int line, char *string, long value){
	...
	sprintf(str,"%ld",value); // convert value to string
	
	if((0 <= line) && (line <= 4)){           // make sure line is in range
		if(device==0){                          // check for device number
			ST7735_SetCursor(0,line);             // device equals to 0, start on line 0 
		}
		else{
			ST7735_SetCursor(0,line+5);           // if device not equal to 0, start on line 6 of the display
		}	
			ST7735_OutString((uint8_t *)string);  // converting from char to uint8_t won't effect the pointer/address
			ST7735_OutString((uint8_t *)str);     // output value
		//printf("%s %ld",string,value);
	}
	
	else{
		// error message
		ST7735_SetCursor(0,12); // row 13
		ST7735_OutString("Lines not within 0-4!");
		//printf("Lines not within 0-4!");
	}
...
// ********************************************************
void ST7735_DrawLine(unsigned short startX, unsigned short startY, unsigned short endX, unsigned short endY, unsigned short color){
    short x0 = startX;
    short x1 = endX;
    short y0 = startY;
    short y1 = endY;
